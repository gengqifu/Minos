...
覆盖以下能力：
- rulesync：本地/远端同步、校验、离线、回滚、清理、超时与错误提示。
...
- 规则包：
  - 本地 tar.gz（由发布流程或测试人员准备）。
  - 远端 HTTP/git/OCI（可使用内网或测试仓库）。
...
- 远端拉取依赖网络，需控制超时与重试次数。
- CI 中不建议运行 rulesync，建议外部预更新缓存。
- 容器内远端拉取需安装 git/oras，并注意证书与代理。

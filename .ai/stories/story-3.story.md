...
2. - [x] 实现测试用例（自动化）  
   ...
   - [x] 2.3 实现 YAML 规则加载/禁用/覆盖/兜底关闭的集成测试（当前以 xfail 标记，待功能实现）  
3. - [x] Manifest 解析与输入处理  
   - [x] 3.1 支持 APK 解包与源码路径直接读取 Manifest  
   - [x] 3.2 处理多变体/多 manifest 合并策略（至少支持主 Manifest）  
4. - [ ] 规则匹配实现（YAML 驱动）  
   - [x] 4.1 从 YAML 加载权限/导出组件规则（pattern/范围/证据提取）；硬编码仅兜底且可禁用/覆盖  
   - [x] 4.2 将核心 Manifest 规则迁移到 YAML（敏感权限、导出组件等），支持禁用/覆盖  
   - [ ] 4.3 关联地区/法规来源标记（来自映射/手动）  
5. - [x] 结果与报告集成  
...
7. - [x] 文档与验收  
   ...
   - [x] 7.2 验收用例：必要权限命中、导出组件命中、无命中、异常 Manifest 处理、来源标记检查

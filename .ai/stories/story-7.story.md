...
4. - [x] GitLab CI 示例  
   ...
   - [x] 4.2 参数示例与缓存/工件收集配置  
5. - [x] 输出与文档  
   - [x] 5.1 在仓库提供 `ci/` 下的示例 YAML/脚本，附运行说明  
   - [x] 5.2 说明如何在受限网络/离线模式下使用本地规则缓存  
6. - [x] 日志与可观测性  
   - [x] 6.1 确保 stdout 摘要清晰；文件日志可作为工件上传  
   - [x] 6.2 退出码约定：配置/运行错误为非零；风险不阻断（无阻断策略）
...
- User: 执行任务4.2  
- Assistant: 在 GitLab CI 示例中明确参数化配置、缓存与工件收集并勾选 4.2
- User: 执行任务5.1  
- Assistant: 添加 ci/README 与主 README 说明，完成输出与文档 5.1
- User: 执行任务5.2  
- Assistant: 补充离线/受限网络规则缓存复用说明并勾选 5.2
- User: 执行任务6.1  
- Assistant: 更新 CI 说明强调 stdout 摘要与文件日志工件，并勾选 6.1
- User: 执行任务6.2  
- Assistant: 补充退出码约定说明并勾选 6.2
- User: 执行任务3.3  

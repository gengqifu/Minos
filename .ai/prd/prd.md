...
- 扫描规则驱动：扫描引擎应从 YAML 规则中读取匹配逻辑（pattern/范围/证据提取等），减少硬编码；保留少量兜底规则但可被 YAML 覆盖/禁用。CLI 扫描应按法规/地区选择从缓存规则中加载：默认从 `~/.minos/rules/<reg>/<version>/rules.yaml` 读取（参数值大小写不敏感），未指定法规时默认使用 PRD “法规参考链接”列出的全部法规；规则缺失/缓存缺失需明确报错并返回非零退出码。支持通过参数指定规则加载目录（`--rules-dir`）以覆盖默认缓存路径。
- 非功能需求：
...
- 约束落地：本地/自定义源默认拒绝，需显式 `--allow-local-sources` 或 `--allow-custom-sources`；metadata 记录 version/source_url/installed_at/active，缓存路径按法规/版本隔离。

...
1. - [ ] 设计测试用例（TDD 先行）  
   1. - [ ] 覆盖：GitHub Actions 工作流、GitLab CI 工作流；产出 HTML/JSON 报告、日志工件；多输入场景（源码/APK）；缺少输入时报错  
   2. - [ ] 断言：步骤成功执行、工件存在且符合命名/路径约定、stdout 摘要包含风险计数/报告路径  
2. - [ ] 实现测试用例（自动化/可执行示例）  
   1. - [ ] 编写/配置可运行的 CI 示例测试（本地 runner/容器模拟）验证工件产出与摘要  
   2. - [ ] 校验退出码、工件路径、报告/schema、缺少输入报错  
3. - [ ] GitHub Actions 示例  
   1. - [ ] 使用官方容器镜像/本地运行方式（可选）  
   2. - [ ] 参数示例：地区/法规、输入路径、输出目录、log-level；缓存规则目录（可选）  
   3. - [ ] 上传工件：HTML/JSON 报告、日志文件  
4. - [ ] GitLab CI 示例  
   1. - [ ] stage/job 配置，镜像或本地运行方式  
   2. - [ ] 参数示例与缓存/工件收集配置  
5. - [ ] 输出与文档  
   1. - [ ] 在仓库提供 `ci/` 下的示例 YAML/脚本，附运行说明  
   2. - [ ] 说明如何在受限网络/离线模式下使用本地规则缓存  
6. - [ ] 日志与可观测性  
   1. - [ ] 确保 stdout 摘要清晰；文件日志可作为工件上传  
   2. - [ ] 退出码约定：配置/运行错误为非零；风险不阻断（无阻断策略）

...
1. - [x] 设计测试用例（TDD 先行）  
   ...
   - [x] 1.4 远端同步断言：下载失败不覆盖旧缓存，错误信息/退出码符合约定  
2. - [x] 实现测试用例（自动化）  
   - [x] 2.1 本地模拟在线源（本地 HTTP/文件）与法规子集参数的测试，验证缓存结构与覆盖策略  
   - [x] 2.2 覆盖校验失败/网络失败/离线缓存的退出码与日志断言  
   - [x] 2.3 远端同步测试实现：HTTP/git/OCI 下载成功/失败/重试/断网覆盖  
...
6. - [x] 远端在线同步实现  
   ...
   - [x] 6.4 容器使用说明：容器内 rulesync 远端 source 示例与网络/证书/缓存注意事项
...
- 远端断网/认证失败：错误提示明确且退出码非零，不影响其他法规缓存；旧缓存不变。
...
- User: 执行任务6.4  
- Assistant: 补充容器内远端 source 示例与网络/证书/缓存注意事项，勾选 6.4

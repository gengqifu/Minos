...
1. - [ ] 设计测试用例（TDD 先行）  
   - [ ] 1.1 覆盖：单地区映射、多地区并集、手动增删法规、默认列表加载、报告来源标记、无效输入/缺省处理  
   - [ ] 1.2 定义断言：映射结果、最终法规集合、来源标记字段、配置持久化/读取、错误提示  
2. - [ ] 实现测试用例（自动化）  
   - [ ] 2.1 编写映射/手动调整/无效输入等场景的测试，含来源标记断言  
   - [ ] 2.2 支持本地与 CI 运行，输出明确的断言与退出码  
3. - [ ] 实现地区/法规数据源加载  
   - [ ] 3.1 加载可选地区与默认映射表（支持扩展/覆盖）  
   - [ ] 3.2 加载可选法规集合，支持自定义扩展  
4. - [ ] 组合逻辑与 API/配置接口  
   - [ ] 4.1 输入：地区列表、多选，返回映射法规并集  
   - [ ] 4.2 高级模式：手动增删法规，输出最终法规集合与来源标记（映射/手动）  
   - [ ] 4.3 配置持久化/读取（如 config 文件或 CLI 参数解析）  
5. - [ ] 报告与下游接口  
   - [ ] 5.1 将地区/法规选择结果和来源标记输出给扫描器/报告生成器  
   - [ ] 5.2 报告字段：地区列表、法规列表、来源标记（地区映射/手动）  
6. - [ ] 日志与校验  
   - [ ] 6.1 stdout 摘要：选定地区、映射法规、手动调整结果  
   - [ ] 6.2 对缺失/无效地区或法规给出清晰错误/警告
7. - [ ] 文档与验收  
   - [ ] 7.1 说明默认映射表、可扩展方式、示例配置/命令  
   - [ ] 7.2 验收用例：单地区、多地区并集、手动添加法规、手动移除法规、报告标记检查

...
- 扫描规则驱动：扫描引擎应从 YAML 规则中读取匹配逻辑（pattern/范围/证据提取等），减少硬编码；保留少量兜底规则但可被 YAML 覆盖/禁用。CLI 扫描应按法规/地区选择从缓存规则中加载：默认从 `~/.minos/rules/<reg>/<version>/rules.yaml` 读取（参数值大小写不敏感），未指定法规时默认使用 PRD “法规参考链接”列出的全部法规；规则缺失/缓存缺失需明确报错并返回非零退出码。
- 非功能需求：

...
3. - [ ] 设计并实现 rulesync CLI  
   - [ ] 3.1 参数：源仓库（git/oci/https）、版本/标签、签名校验开关、缓存目录（默认 `~/.minos/rules` 可配）  
   - [ ] 3.2 拉取并解压规则包；校验 SHA256（预留 GPG）；写入 metadata（版本、来源、校验结果、时间戳）  
   - [ ] 3.3 失败与重试策略（网络/校验失败的错误提示与非零退出码）  
4. - [ ] 缓存与回滚机制  
   - [ ] 4.1 维护规则版本列表与激活版本指针  
   - [ ] 4.2 提供回滚到上一版本/指定版本的命令或参数  
   - [ ] 4.3 清理旧版本/缓存（限额或按版本数）  
5. - [ ] 与扫描器的接口输出  
   - [ ] 5.1 生成 machine-readable 的 metadata.json（版本、签名、来源 URL、生成时间、激活标记）  
   - [ ] 5.2 输出当前激活规则路径供扫描器读取并校验存在性  
6. - [ ] 日志与可观测性  
